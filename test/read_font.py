...
class StrokeFonts():
    ...
    def __init__(self):
        font_file = open("../data/KST32Bv3/KST32B.TXT", 'r', encoding='shift_jis')
...
def main():
    rate = 10
    stroke_fonts = StrokeFonts()
    lines_data = stroke_fonts.get_lines_data('論')  # ｐ
    img = np.ones((32 * rate, 32 * rate), np.uint8) * 255
    test_strings = 'あアaａAＡ1１亜'
    for charactor in test_strings:
        stroke_fonts.preview_font_line(charactor,0.3)
    # End for
    # stroke_fonts.all_font_save_img()
# End def
